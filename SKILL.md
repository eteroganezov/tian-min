---
name: bazi-ziwei
description: 八字 + 紫微斗数 AI 排盘与综合分析。当用户提供生辰（本地公历日期、当地钟表时间、性别）询问八字、紫微、命理、命盘、流年大运相关问题时使用。基于 Yiqi 算法库 + enrichBazi 补全层精准排盘（杜绝 LLM 自行排盘出错），支持按需独立分析八字 / 独立分析紫微 / 两盘交叉印证。
---
...
> 当前 MVP 仅支持公历日期与出生地当地钟表时间。不支持农历输入、时区自动换算或真太阳时经度校正。
...
| 现象 | 原因 | 处理 |
|---|---|---|
| 排盘脚本报错 | 日期超 1900-2100 / 日期不存在 / 时辰或性别格式错 | 询问用户校正 |
| 用户提供农历或要求时区/真太阳时换算 | 当前 MVP 不支持 | 明确告知限制，请用户提供本地公历日期和当地钟表时间 |
