...
function getYinYang(yearGan: string, gender: 'male' | 'female'): string {
  ...
  if (ganIndex < 0) throw new Error(`无效年干: ${yearGan}`);
...
export function runZiweiTests() {
  const cases = [
    {
      ...
      input: { year: 2000, month: 1, day: 1, hour: 12, minute: 0, gender: 'male' as const, isLunar: false, timeZone: 8 },
      expected: { lunarYear: 1999, yearGan: '己', yinYang: '阴男', ming: '午', shen: '午', ju: '土五局' }
    },
    {
      name: '2000-02-05 男：中国新年后庚辰年应为阳男',
      input: { year: 2000, month: 2, day: 5, hour: 12, minute: 0, gender: 'male' as const, isLunar: false, timeZone: 8 },
  ...
  for (const testCase of cases) {
    try {
      ...
      const ok = Object.entries(testCase.expected).every(([key, value]) => actual[key as keyof typeof actual] === value);
      if (ok) passed++; else failed++;
      details.push({ name: testCase.name, passed: ok, expected: testCase.expected, actual });
    } catch (error) {
      failed++;
      details.push({ name: testCase.name, passed: false, error: (error as Error).message });
    }
  }
