...
function main() {
  ...
  if (args.gender === 'male' || args.gender === '男') gender = 'male';
  else if (args.gender === 'female' || args.gender === '女') gender = 'female';
  else throw new Error('Некорректный пол: используйте male/female или 男/女');

  // Step 1: Yiqi 算法层 — 四柱+紫微+大运+流年
  const chart: any = calculateLocalChart({
