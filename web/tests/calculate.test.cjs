...
test("production build содержит бесплатную воронку и нормализованную терминологию", () => {
  ...
  assert.match(script, /четыре столпа/i);
  ...
  assert.match(script, /Посмотреть 12 дворцов/);
  assert.match(script, /Скрыть 12 дворцов/);
  assert.match(script, /Нажмите на дворец, чтобы узнать значение жизненной сферы/);
  assert.match(script, /Две карты рассчитаны/);
