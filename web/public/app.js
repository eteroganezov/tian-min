...
function renderExecutivePortrait(data) { return `<section class="report-section lead executive-portrait"><p class="section-label">Главное за минуту</p><h2>Ваш личный портрет</h2><div class="prose narrow">${paragraphs(data.summary)}</div><div class="portrait-features">${data.keyFeatures.map((item,i)=>`<article><span>0${i+1}</span><p>${e(item)}</p></article>`).join("")}</div><div class="portrait-compass"><article><span>Главный ресурс</span><p>${e(data.mainResource)}</p></article><article><span>Главный риск</span><p>${e(data.mainRisk)}</p></article><article><span>Тема периода</span><p>${e(data.currentTheme)}</p></article></div></section>`; }
function renderEditorialSection(data, cls) { return `<section class="report-section editorial-section ${e(cls)}"><p class="section-label">${e(data.title)}</p><h2>${e(data.headline)}</h2><div class="prose narrow">${paragraphs(data.summary)}</div><div class="editorial-grid"><article><span>Что это даёт</span>${list(data.strengths)}</article><article><span>Зона риска</span>${list(data.risks)}</article><article><span>Что делать</span>${ordered(data.actions)}</article></div><div class="key-points">${data.keyPoints.map(item=>`<p>${e(item)}</p>`).join("")}</div>${data.evidence.length ? `<details class="evidence"><summary>Основания в карте</summary>${list(data.evidence)}</details>` : ""}<p class="confidence-note">${e(data.confidenceNote)}</p></section>`; }
