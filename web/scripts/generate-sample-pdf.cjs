...
function buildLongStressVariant() {
  ...
  return { key:"long",filename:"sample-personal-report-v4-long.pdf",...buildVariant({ input,reportYears:currentReportYears(),mutate:report=>{
      ...
      report.scenarios[0].title="Последовательное согласование долгосрочных приоритетов, критериев качества и границ личной ответственности";
      report.lifeAreaMatrix[0].synthesis+=" Дополнительная проверка показывает, что общий вывод должен оставаться видимым целиком даже при более подробном объяснении взаимосвязи между исходными признаками, рабочей средой и способом принимать решения.";
    } }) };
}
