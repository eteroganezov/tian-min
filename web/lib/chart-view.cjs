...
function toChartView(chart) {
  ...
  return {
    ...
    bazi: {
      ...
      strength: { verdict: bazi.enrichment.旺衰.verdict, score: bazi.enrichment.旺衰.score, confidence: bazi.enrichment.旺衰.confidence },
    ...
    ziwei: {
      ...
      palaces: ziwei.gongs.map(gong => ({
        name: gong.gong.endsWith("宫") ? gong.gong : `${gong.gong}宫`, dizhi: gong.dizhi,
