...
function legacyManifestations(doc,data){if(!data?.items?.length)return;page(doc,"Практическая самопроверка","Сверьте выводы с вашей реальной жизнью.");data.items.forEach((item,index)=>manifestationRow(doc,item,index));}
function legacyActionPlan(doc,data){if(!data)return;page(doc,data.title,"Короткий список действий, которые помогают перевести выводы в практику.",{kicker:"ПРАКТИЧЕСКИЙ ИТОГ"});actionPlanColumns(doc,data.items||[]);}
...
function parseMatrixItem(text){const source=String(text).replace(/\u00a0/g," ");const areas=["Карьера","Финансы","Отношения","Самовыражение","Окружение","Внутреннее состояние","Дом и перемены","Здоровье"];const area=areas.find(x=>source.startsWith(x));if(!area&&/^Цзы Вэй Доу Шу\s+/u.test(source))return{area:"Здоровье",alignment:"",sourceLabel:"Данные Цзы Вэй",text:source};if(!area)return null;const rest=source.slice(area.length).trim();const alignment=["Согласие","Дополнение","Расхождение"].find(x=>rest.startsWith(x));if(!alignment)return{area,alignment:"",sourceLabel:"",text:rest};return{area,alignment,sourceLabel:"",text:rest.slice(alignment.length).trim()};}
...
function evidenceMetrics(doc,evidence){doc.font(doc._tianMingBrandFont).fontSize(6.2);const heights=evidence.map(line=>Math.max(8,doc.heightOfString(line,{width:340,lineGap:1})));return{heights,contentHeight:heights.reduce((sum,height)=>sum+height,0)+Math.max(0,heights.length-1)*3};}
function evidenceBlock(doc,evidence,y,metrics){doc.moveTo(70,y-7).lineTo(521,y-7).strokeColor("#d8ded9").stroke();doc.fillColor(colors.gold).font("Bold").fontSize(6.2).text("ОСНОВАНИЕ В КАРТЕ",70,y,{width:100,lineGap:1});let lineY=y;evidence.forEach((line,index)=>{doc.fillColor(colors.muted).font(doc._tianMingBrandFont).fontSize(6.2).text(line,181,lineY,{width:340,lineGap:1});lineY+=metrics.heights[index]+3;});}
function traitCard(doc,item,index,minHeight=228){const evidence=splitEvidence(item.evidence);const compact=minHeight<=200;doc.font("Body").fontSize(8.8);const descriptionHeight=doc.heightOfString(validConsumerText(item.description),{width:451,lineGap:2});const splitOffset=Math.max(compact?72:88,44+descriptionHeight+12);const panelFontSize=compact?7.7:8.1;doc.font("Body").fontSize(panelFontSize);const panelBodyHeight=Math.max(...[item.strong,item.low].map(text=>doc.heightOfString(validConsumerText(text),{width:194,lineGap:2})),0);const panelHeight=Math.max(compact?72:82,28+panelBodyHeight+12);const evidenceInfo=evidenceMetrics(doc,evidence);const evidenceOffset=splitOffset+panelHeight+14;const naturalHeight=evidence.length?evidenceOffset+Math.max(10,evidenceInfo.contentHeight)+14:splitOffset+panelHeight+14;const height=Math.max(minHeight,naturalHeight);ensure(doc,height+12);const y=doc.y;doc.roundedRect(52,y,491,height,9).fill("#f8f7f2");doc.fillColor(colors.gold).font("Bold").fontSize(9).text(String(index+1).padStart(2,"0"),70,y+17,{width:22});doc.fillColor(colors.ink).font("Bold").fontSize(15).text(clean(item.title),99,y+13,{width:422});doc.fillColor(colors.ink).font("Body").fontSize(8.8).text(validConsumerText(item.description),70,y+44,{width:451,lineGap:2});const splitY=y+splitOffset;[[70,colors.sage,colors.jade,"СИЛЬНАЯ СТОРОНА",item.strong],[299,"#f3e9df",colors.gold,"КОГДА РЕСУРСА МАЛО",item.low]].forEach(([x,bg,accent,label,text])=>{doc.roundedRect(x,splitY,222,panelHeight,6).fill(bg);doc.rect(x,splitY,4,panelHeight).fill(accent);doc.fillColor(accent).font("Bold").fontSize(7).text(label,x+14,splitY+11,{width:194});doc.fillColor(colors.ink).font("Body").fontSize(panelFontSize).text(validConsumerText(text),x+14,splitY+28,{width:194,lineGap:2});});if(evidence.length)evidenceBlock(doc,evidence,y+evidenceOffset,evidenceInfo);doc.y=y+height+12;}
...
function actionColumnData(doc,item,index){const prefix=index?"Избегать":"Делать чаще";const text=String(item||"").replace(new RegExp(`^${prefix}\\s+`),"");const values=sentences(text).map(validConsumerText).filter(Boolean);doc.font("Body").fontSize(8.7);const heights=values.map(value=>Math.max(64,doc.heightOfString(value,{width:195,lineGap:2.5})+18));return{label:index?"Чего избегать":"Делать чаще",values,heights};}
function actionPlanColumns(doc,items){const columns=[0,1].map(index=>actionColumnData(doc,items[index],index));const y=230,w=239;const contentTop=y+50;const contentHeights=columns.map(column=>column.heights.reduce((sum,height)=>sum+height,0));const dividerBottom=contentTop+Math.max(...contentHeights,0)-8;doc.moveTo(303,contentTop-8).lineTo(303,dividerBottom).lineWidth(1).strokeColor("#e3e4df").stroke();columns.forEach((column,index)=>{const x=52+index*251,accent=index?colors.red:colors.jade;doc.fillColor(accent).font("Bold").fontSize(14).text(column.label,x,y,{width:w});doc.moveTo(x,y+26).lineTo(x+w-12,y+26).lineWidth(2).strokeColor(accent).stroke();let itemY=contentTop;column.values.forEach((value,itemIndex)=>{doc.circle(x+13,itemY+7,13).fill(itemIndex%2?colors.sand:colors.sage);doc.fillColor(colors.gold).font("Bold").fontSize(7.5).text(String(itemIndex+1).padStart(2,"0"),x+2,itemY+2,{width:22,align:"center"});doc.fillColor(colors.ink).font("Body").fontSize(8.7).text(value,x+38,itemY-1,{width:w-44,lineGap:2.5});itemY+=column.heights[itemIndex];});});}
...
function ziweiVisualPages(doc, chart, hasCjk) {
  ...
  doc.fillColor(colors.jade).font("Bold").fontSize(8.8).text("12 ДВОРЦОВ ЦЗЫ ВЭЙ",52,496,{width:491,characterSpacing:.5});
  }
  palaces.forEach((palace, index) => {
    const col = index % 2, row = Math.floor(index / 2);
    const x = 52 + col * 251, y = (pageIndex ? 145 + row * 130 : 524 + row * 121);
...
function cleanNarrative(value) {
  return clean(value)
    ...
    .replace(/Структура\s+«Прямой чиновник»\s+связана\s+с\s+чувствительность\s+к\s+стандарту/giu,"Структура «Прямой чиновник» связана с чувствительностью к стандарту")
    .replace(/Тянь\s+Тун\s+часто\s+связывают\s+с\s+потребность\s+в\s+человеческом/giu,"Тянь Тун часто связывают с потребностью в человеческом")
    ...
    .replace(/Символически\s+это\s+сочетание\s+рыночных\s+возможностей\s+с\s+конкуренцией,\s+равными\s+партнёрами\s+и\s+необходимостью\s+делить\s+ресурс\./giu,"Это сочетание связывают с рыночными возможностями, конкуренцией, партнёрством и необходимостью делить ресурсы.")
