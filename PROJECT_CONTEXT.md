...
Premium is a separate one-time purchase: **«Ба-цзы + Цзы Вэй · Персональный разбор · Полный PDF-отчёт»**. The current temporary closed early-user test price is **399 RUB** (`39900` minor units); the previous 599 RUB test price is not the current source of truth. Production price is server-owned; the frontend only displays `/api/premium/config`. The default development/mock price may remain `100 RUB` when no explicit development override is set.
...
- Current Premium price is 399 RUB for the temporary closed early-user test and remains server-controlled.
